...
class Client {
    constructor(username = "guest", password = "guest", options = { useWebSockets: false, camYou: false }) {
        ...
        switch (packet.FCType) {
            case Constants_1.FCTYPE.LOGIN:
                if (packet.nArg1 !== 0) {
                    ...
                    this.TxCmd(Constants_1.FCTYPE.ROOMDATA, 0, 1, 0);
                }
                break;
                ...
                if (packet.nArg1 === 0 && packet.nArg2 === 0) {
                    if (Array.isArray(packet.sMessage)) {
                        for (let i = 0; i < packet.sMessage.length; i = i + 2) {
                            let possibleModel = Model_1.Model.getModel(packet.sMessage[i]);
                            if (possibleModel !== undefined) {
    ...
    ensureEmoteParserIsLoaded() {
        return __awaiter(this, void 0, void 0, function* () {
            if (this.emoteParser === undefined) {
                let obj = yield this.loadFromMFC(`http://www.${this.baseUrl}/_js/mfccore.js`, (content) => {
    ...
    ensureServerConfigIsLoaded() {
        return __awaiter(this, void 0, void 0, function* () {
            if (this.serverConfig === undefined) {
                let obj = yield this.loadFromMFC(`http://www.${this.baseUrl}/_js/serverconfig.js?nc=${Math.random()}`, (text) => {
    ...
    TxCmd(nType, nTo = 0, nArg1 = 0, nArg2 = 0, sMsg) {
        Utils_1.logWithLevel(Utils_1.LogLevel.VERBOSE, "TxCmd Sending - nType: " + Constants_1.FCTYPE[nType] + ", nTo: " + nTo + ", nArg1: " + nArg1 + ", nArg2: " + nArg2 + ", sMsg:" + sMsg);
        ...
        return new Promise((resolve, reject) => {
            ...
            this.ensureServerConfigIsLoaded().then(() => {
                if (!this.useWebSockets) {
                    ...
                    Utils_1.logWithLevel(Utils_1.LogLevel.INFO, `Connecting to ${this.camYou ? "CamYou" : "MyFreeCams:"} chat server ${chatServer}...`);
                    this.client = this.net.connect(8100, chatServer + `.${this.baseUrl}`, () => {
    ...
    login(username, password) {
        ...
        if (!this.useWebSockets) {
            this.TxCmd(Constants_1.FCTYPE.LOGIN, 0, 20071025, 0, localUsername + ":" + this.password);
        }
        else {
            this.TxCmd(Constants_1.FCTYPE.LOGIN, 0, 20080909, 0, localUsername + ":" + this.password);
        }
    }
