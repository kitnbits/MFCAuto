...
class Client extends events_1.EventEmitter {
    ...
    constructor(username = "guest", password = "guest", options = {}) {
        ...
        if (typeof options === "boolean") {
            Utils_1.logWithLevel(Utils_1.LogLevel.WARNING, `WARNING: Client useWebSockets as a boolean third constructor parameter is being deprecated, please see the release notes for v4.2.0 for the current way to use a websocket server connection`);
        ...
        Utils_1.logWithLevel(Utils_1.LogLevel.DEBUG, `[CLIENT] Constructed, State: ${this._state}`);
    }
    ...
    removeAllListeners(event) {
        Utils_1.logWithLevel(Utils_1.LogLevel.WARNING, `WARNING: Using Client.removeAllListeners may break MFCAuto, which internally adds its own listeners at times`);
            ...
            case constants.FCTYPE.EXTDATA:
                if (packet.nTo === this.sessionId && packet.nArg2 === constants.FCWOPT.REDIS_JSON) {
                    this._handleExtData(packet.sMessage).catch((reason) => {
                        Utils_1.logWithLevel(Utils_1.LogLevel.DEBUG, `[CLIENT] _packetReceived caught rejection from _handleExtData: ${reason}`);
                    });
                }
                break;
    ...
    _handleExtData(extData) {
        return __awaiter(this, void 0, void 0, function* () {
            if (extData !== undefined && extData.respkey !== undefined) {
                ...
                let contents = "";
                try {
                    contents = yield Utils_1.httpsGet(url);
    ...
    _loadFromMFC(url, massager) {
        return __awaiter(this, void 0, void 0, function* () {
            let contents = yield Utils_1.httpsGet(url);
        ...
        return __awaiter(this, void 0, void 0, function* () {
            if (this._serverConfig === undefined) {
                if (this._options.useCachedServerConfig) {
                    ...
                    const mfcConfig = yield Utils_1.httpsGet(`https://www.${this._baseUrl}/_js/serverconfig.js?nc=${Math.random()}`);
                    try {
                        this._serverConfig = JSON.parse(mfcConfig);
                    }
                    catch (e) {
                        Utils_1.logWithLevel(Utils_1.LogLevel.ERROR, `Error parsing serverconfig: '${mfcConfig}'`);
                        throw e;
                    }
                }
            }
    ...
    TxCmd(nType, nTo = 0, nArg1 = 0, nArg2 = 0, sMsg) {
        Utils_1.logWithLevel(Utils_1.LogLevel.DEBUG, `TxCmd Sending - nType: ${constants.FCTYPE[nType]}, nTo: ${nTo}, nArg1: ${nArg1}, nArg2: ${nArg2}, sMsg:${sMsg}`);
    ...
    static toUserId(id) {
        ...
        else if (id >= constants.CAMCHAN.ID_START) {
            id = id - constants.CAMCHAN.ID_START;
        }
        else if (id >= 300000000) {
            id = id - 300000000;
        }
        else if (id >= constants.SESSCHAN.ID_START) {
            id = id - constants.SESSCHAN.ID_START;
        }
        else if (id >= constants.CHANNEL.ID_START) {
    ...
    connect(doLogin = true) {
        return __awaiter(this, void 0, void 0, function* () {
            Utils_1.logWithLevel(Utils_1.LogLevel.DEBUG, `[CLIENT] connect(${doLogin}), state: ${exports.ClientState[this._state]}`);
            if (this._state === exports.ClientState.PENDING) {
                ...
                Utils_1.logWithLevel(Utils_1.LogLevel.DEBUG, `[CLIENT] State: ${this._state}`);
                return new Promise((resolve, reject) => {
                    ...
                    this._ensureServerConfigIsLoaded().then(() => {
                        if (!this._options.useWebSockets) {
                            ...
                            const chatServer = this._serverConfig.chat_servers[Math.floor(Math.random() * this._serverConfig.chat_servers.length)];
                            Utils_1.logWithLevel(Utils_1.LogLevel.INFO, `Connecting to ${this._options.camYou ? "CamYou" : "MyFreeCams:"} chat server ${chatServer}...`);
                            ...
                            // communicating with the MFC chat servers
                            const wsSrvs = Object.getOwnPropertyNames(this._serverConfig.websocket_servers);
                            ...
                            this._client.on("open", () => {
                                this._client.send("hello fcserver\n\0");
                                // Connecting without logging in is the rarer case, so make the default to log in
                                if (doLogin) {
                                    this._disconnectIfNo(constants.FCTYPE.LOGIN, this._options.loginTimeout, "Server did not respond to the login request, retrying");
                                    this.login();
     ...
     */
    _keepAlive() {
        Utils_1.logWithLevel(Utils_1.LogLevel.DEBUG, `[CLIENT] _keepAlive() ${this._state}/${this._currentConnectionStartTime}`);
        if (this._state === exports.ClientState.ACTIVE && this._currentConnectionStartTime) {
            ...
            if (lastPacketDuration > this._options.silenceTimeout
                ...
                if (this._client !== undefined) {
                    Utils_1.logWithLevel(Utils_1.LogLevel.DEBUG, `[CLIENT] _keepAlive silence tripped, lastPacket: ${lastPacketDuration}, lastStatePacket: ${lastStatePacketDuration}`);
                    const msg = `Server has not responded for too long, forcing disconnect`;
                    Utils_1.logWithLevel(Utils_1.LogLevel.INFO, msg);
                    try {
                        if (this._client instanceof net.Socket) {
                            this._client.end();
                        }
    ...
    _disconnected(reason) {
        if (this._state !== exports.ClientState.IDLE) {
            Utils_1.logWithLevel(Utils_1.LogLevel.INFO, `Disconnected from ${this._baseUrl} - ${reason}`);
            this._completedModels = false;
            this._completedTags = false;
            ...
            if (this._choseToLogIn === true && Client._connectedClientCount > 0) {
                ...
                Utils_1.logWithLevel(Utils_1.LogLevel.DEBUG, `[CLIENT] connectedClientCount: ${Client._connectedClientCount}`);
            }
            ...
            if (!this._manualDisconnect) {
                this._state = exports.ClientState.PENDING;
                Utils_1.logWithLevel(Utils_1.LogLevel.DEBUG, `[CLIENT] State: ${this._state}`);
                Utils_1.logWithLevel(Utils_1.LogLevel.INFO, `Reconnecting in ${Client._currentReconnectSeconds} seconds...`);
            ...
            Utils_1.logWithLevel(Utils_1.LogLevel.DEBUG, `[CLIENT] emitting: CLIENT_DISCONNECTED, _choseToLogIn: ${this._choseToLogIn}`);
    ...
    login(username, password) {
        ...
        Utils_1.logWithLevel(Utils_1.LogLevel.DEBUG, `[CLIENT] _connectedClientCount: ${Client._connectedClientCount}`);
    ...
    disconnect() {
        return __awaiter(this, void 0, void 0, function* () {
            Utils_1.logWithLevel(Utils_1.LogLevel.DEBUG, `[CLIENT] disconnect(), state: ${exports.ClientState[this._state]}`);
            if (this._state !== exports.ClientState.IDLE) {
                return new Promise((resolve) => {
