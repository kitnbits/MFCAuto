...
class Client extends events_1.EventEmitter {
    ...
    _packetReceived(packet) {
        ...
        switch (packet.FCType) {
            case constants.FCTYPE.LOGIN:
                ...
                if (packet.nArg1 !== 0) {
                    Utils_1.logWithLevelInternal(Utils_1.LogLevel.ERROR, `Login failed for user '${this.username}' password '${this.password}'`);
    ...
    _disconnected(reason) {
        if (this._state !== exports.ClientState.IDLE) {
            Utils_1.logWithLevelInternal(Utils_1.LogLevel.INFO, `Disconnected from ${this._baseUrl} - ${reason}`);
            this._completedModels = false;
            this._completedTags = false;
