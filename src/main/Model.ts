...
export class Model extends EventEmitter {
    ...
    public addListener(event: ModelEventName, listener: ModelEventCallback) {
     ...
     * client.connect();
     */
    public on(event: ModelEventName, listener: ModelEventCallback) {
     ...
     * change, and the value of the property after the change.
     */
    public once(event: ModelEventName, listener: ModelEventCallback) {
        return super.once(event, listener);
    }
    public prependListener(event: ModelEventName, listener: ModelEventCallback) {
        return super.prependListener(event, listener);
    }
    public prependOnceListener(event: ModelEventName, listener: ModelEventCallback) {
     ...
     */
    public removeListener(event: ModelEventName, listener: ModelEventCallback) {
        return super.removeListener(event, listener);
    }
    public removeAllListeners(event?: ModelEventName) {
    ...
    public listeners(event: ModelEventName) {
        return super.listeners(event) as ModelEventCallback[];
    }
    public emit(event: ModelEventName, ...args: Array<UnknownJsonField | string[]>) {
        return super.emit(event, ...args);
    }
    public eventNames() {
        return super.eventNames() as ModelEventName[];
    }
    public listenerCount(type: ModelEventName) {
        return super.listenerCount(type);
    }
    public rawListeners(event: ModelEventName) {
        return super.rawListeners(event) as ModelEventCallback[];
    }
    // #endregion

    // #region Static EventEmitter methods
    // EventEmitter object to be used for events firing for all models
    private static readonly eventsForAllModels: EventEmitter = new EventEmitter();

    // Expose the "all model" events as constructor properies to be accessed
    // like Model.on(...)
    public static addListener = Model.eventsForAllModels.addListener.bind(Model.eventsForAllModels) as (event: ModelEventName, listener: ModelEventCallback) => EventEmitter;
    /**
     * [EventEmitter](https://nodejs.org/api/all.html#events_class_eventemitter)
     ...
     * client.connect();
     */
    public static on = Model.eventsForAllModels.on.bind(Model.eventsForAllModels) as (event: ModelEventName, listener: ModelEventCallback) => EventEmitter;
    /**
     * [EventEmitter](https://nodejs.org/api/all.html#events_class_eventemitter)
     * method like Model.on but the registered callback is only invoked once,
     * on the first instance of the given event
     ...
     * before the change, and the value of the property after the change:
     */
    public static once = Model.eventsForAllModels.once.bind(Model.eventsForAllModels) as (event: ModelEventName, listener: ModelEventCallback) => EventEmitter;
    public static prependListener = Model.eventsForAllModels.prependListener.bind(Model.eventsForAllModels) as (event: ModelEventName, listener: ModelEventCallback) => EventEmitter;
    public static prependOnceListener = Model.eventsForAllModels.prependOnceListener.bind(Model.eventsForAllModels) as (event: ModelEventName, listener: ModelEventCallback) => EventEmitter;
    /**
     * [EventEmitter](https://nodejs.org/api/all.html#events_class_eventemitter)
     * method that removes a listener callback previously registered with
     * Model.on or Model.once
     */
    public static removeListener = Model.eventsForAllModels.removeListener.bind(Model.eventsForAllModels) as (event: ModelEventName, listener: ModelEventCallback) => EventEmitter;
    public static removeAllListeners = Model.eventsForAllModels.removeAllListeners.bind(Model.eventsForAllModels) as (event?: ModelEventName) => EventEmitter;
    public static getMaxListeners = Model.eventsForAllModels.getMaxListeners.bind(Model.eventsForAllModels) as () => number;
    public static setMaxListeners = Model.eventsForAllModels.setMaxListeners.bind(Model.eventsForAllModels) as (n: number) => EventEmitter;
    public static listeners = Model.eventsForAllModels.listeners.bind(Model.eventsForAllModels) as (event: ModelEventName) => ModelEventCallback[];
    public static emit = Model.eventsForAllModels.emit.bind(Model.eventsForAllModels) as (event: ModelEventName, ...args: Array<UnknownJsonField | string[]>) => boolean;
    public static eventNames = Model.eventsForAllModels.eventNames.bind(Model.eventsForAllModels) as () => ModelEventName[];
    // tslint:disable-next-line:no-unsafe-any
    public static listenerCount = Model.eventsForAllModels.listenerCount.bind(Model.eventsForAllModels); // as (type: ModelEventName) => number;
    public static rawListeners = Model.eventsForAllModels.rawListeners.bind(Model.eventsForAllModels) as (event: ModelEventName) => ModelEventCallback[];
    // #endregion
     ...
     * ID. The [first example here](https://github.com/ZombieAlex/MFCAuto/blob/master/README.md)
        ...
        for (const key in msg) {
            ...
            if (key === "u" || key === "m" || key === "s") {
                ...
                if (typeof details === "object") {
                    for (const key2 in details) {
                        ...
                        if (typeof details[key2] === "string") {
                            details[key2] = decodeIfNeeded(details[key2] as string);
                        }
                    ...
                    for (const key2 in sites) {
                        ...
                        if (typeof sites[key2] === "object") {
                            ...
                            for (const key3 in siteObject) {
                                ...
                                if (typeof siteObject[key3] === "string") {
                                    siteObject[key3] = decodeIfNeeded(siteObject[key3] as string);
                                }
                ...
                if (typeof msg[key] === "string") {
                    msg[key] = decodeIfNeeded(msg[key] as string);
                }
    ...
    public async getSocialMedia(): Promise<ModelSocialMedia | undefined> {
        ...
        try {
            rawContents = await httpsGet(url);
            // tslint:disable-next-line:no-unsafe-any
