    ...
    describe(name, function () {
        describe("Startup Scenarios", function () {
            ...
            it("should be able to dynamically load the server config", function (done) {
                ...
                client._ensureServerConfigIsLoaded().then(() => {
                    assert.notStrictEqual(client._serverConfig, undefined);
                    assert.notStrictEqual(client._serverConfig.chat_servers, undefined);
                    assert.notStrictEqual(client._serverConfig.chat_servers.length, 0);
                    done();
                })
                .catch((reason) => {
                    done(reason);
            ...
            it("should be able to log in as two guests", function (done) {
                client = new mfc.Client("guest", "guest", clientOptions);
                let client2 = new mfc.Client("guest", "guest", clientOptions);
                assert.strictEqual(mfc.Client._connectedClientCount, 0);
                assert.strictEqual(client.state, mfc.ClientState.IDLE, "Should not be connected");
        ...
        describe("Reconnect Scenarios", function () {
            ...
            it("should succeed on the initial connect once chat server DNS recovers {slow}", function (done) {
                ...
                client._ensureServerConfigIsLoaded()
                    .then(() => {
                        originalServerConfig = client._serverConfig;
                        if (clientOptions.useWebSockets === false) {
                            client._serverConfig.chat_servers = ["thiswillnotexist"];
                        } else {
                            client._serverConfig.websocket_servers = {"thiswillnotexist":"rfc6455"};
                        }
                    });
                timer = setTimeout(() => client._serverConfig = originalServerConfig, randInt(6000, 7000));
                client.connect()
                    .then(() => {
                        done();
                    })
            ...
            it("should succeed on reconnect once chat server DNS recovers {slow}", function (done) {
                ...
                client.connect()
                    .then(() => {
                        const originalServerConfig = Object.assign({}, client._serverConfig);
                        if (clientOptions.useWebSockets === false) {
                            client._serverConfig.chat_servers = ["thiswillnotexist"];
                        } else {
                            client._serverConfig.websocket_servers = {"thiswillnotexist":"rfc6455"};
                        ...
                        client.once("CLIENT_DISCONNECTED", () => {
                            timer = setTimeout(() => {
                                client._serverConfig = originalServerConfig;
                            }, randInt(6000, 7000));
                            client.ensureConnected()
                                .then(() => {
                                    done();
