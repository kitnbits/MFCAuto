...
[{}, {useWebSockets: true}, {camYou: true}, {useWebSockets: true, camYou: true}].forEach(function (clientOptions) {
    describe("Startup Scenarios: " + JSON.stringify(clientOptions), function () {
        this.timeout(9000);
        let client = null;
        afterEach((done) => {
    ...
    describe("Connected Scenarios: " + JSON.stringify(clientOptions), function () {
        this.timeout(9000);
        ...
        describe("Client", () => {
            ...
            it("should reject if joinRoom fails", (done) => {
        ...
        describe("Model", function () {
            ...
            it("should be able to listen for a specific model state change (this test frequently times out)", function(done) {
                mfc.Model.getModel(queen.uid).on("rc", (model/*, oldstate, newstate*/) => {
    ...
    describe("Reconnect Scenarios: " + JSON.stringify(clientOptions), function () {
        this.timeout(30000);
        let client = null;
        afterEach((done) => {
            client.disconnect().then(() => {
        ...
        it("should recover from a socket disconnect", (done) => {
            ...
            client.on("LOGIN", () => {
                if (firstConnect) {
                    ...
                    if (!clientOptions.useWebSockets) {
                        setTimeout(() => client.client.end(), randInt(50, 1500));
                    } else {
                        setTimeout(() => client.client.close(), randInt(50, 1500));
                    }
                } else {
